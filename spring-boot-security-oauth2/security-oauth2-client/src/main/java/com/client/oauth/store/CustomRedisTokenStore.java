 ...
 *  <p>本来框架已经给我们提供 {@link RedisTokenStore}类了，但是我们角色这个类还需要个性化一些，所以我们就自定义了这个
 *  类。</p>
 * @author: stars
 * @data: 2020年 10月 05日 18:16
 **/
public class CustomRedisTokenStore implements TokenStore {
    ...
    private int getAccessTokenValiditySeconds(String clientId) {
        ...
        if (bytes != null) {
            ...
            // 客户端令牌的有效期。如果未显式设置，则为空（实现可能使用该事实为实例提供默认值）。
        ...
        //返回默认值
        return securityProperties.getAuth().getRenew().getAccessTokenValiditySeconds();
    }
